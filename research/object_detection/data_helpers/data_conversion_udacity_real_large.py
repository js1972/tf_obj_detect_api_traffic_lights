...
def main(_):
    ...
    INPUT_YAML = "object_detection/data/rosbag_traffic_light_data/sloth_udacity_real_data.yaml"
    examples = yaml.load(open(INPUT_YAML, 'rb').read())

    # Split annotations into training and validation datasets
    # Create training record    
    len_examples = len(examples)
    print('Total examples:', len_examples)
    random.shuffle(examples)
    train_examples = examples[:int(len_examples * 0.8)]  # for training (70%)
    ...
    val_examples = examples[int(len_examples * 0.8):]
